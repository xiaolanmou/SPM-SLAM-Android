 ...
 */
double
orthogonal_iteration(matd_t **v, matd_t **p, matd_t **t, matd_t **R, int n_points, int n_steps) {
...
matd_t *fix_pose_ambiguities(matd_t **v, matd_t **p, matd_t *t, matd_t *R, int n_points) {
    ...
    for (int i = 0; i < n_roots; i++) {
        ...
        if (a2 - 2 * a0 + (3 * a3 - 6 * a1) * t1 + (6 * a4 - 8 * a2 + 10 * a0) * t2 +
            (-8 * a3 + 6 * a1) * t3 + (-6 * a4 + 3 * a2) * t4 + a3 * t5 >= 0) {
...
void estimate_tag_pose_orthogonal_iteration(
    ...
    for (int i = 0; i < 4; i++) {
        v[i] = matd_create_data(3, 1, (double[]) {
                (info->det->p[i][0] - info->cx) / info->fx,
