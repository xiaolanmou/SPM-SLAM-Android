...
//    Fy = 400;
    ...
    KF_init = false;
    change_color = false;
    trajectory_color.emplace_back(255, 0, 0);
    indexs.push_back(0);
    look_down = false;
    Ground_idx=0;
}
...
cv::Scalar DrawResult::newColor() {
    ...
    return { fR*255, fG*255, fB*255 };
}
vector<Vector3f> DrawResult::calculate_camera_pose(const Vector3f& camera_center, Matrix3f Rc, float length)
{
    vector<Vector3f> result;
    vector<Vector3f> origin;
    origin.emplace_back(length/2.0,length/2.0,-length*0.7);
    origin.emplace_back(-length/2.0,length/2.0,-length*0.7);
    origin.emplace_back(-length/2.0,-length/2.0,-length*0.7);
    origin.emplace_back(length/2.0,-length/2.0,-length*0.7);
...
{
    int iterNum = 2000, k = 1;    //max iterate num and current iterate num
    float sigma = 0.01;
    int pretotal = 0;
    Vector4f bestplane;
    bestplane << 0,0,0,0;

    while(pretotal<point_cloud.size()/2 && k < iterNum)
    {
...
Vector3f DrawResult::findGround(vector<Vector3f> &point_cloud, vector<Vector3f> &inlier_points) const
{
    if(!look_down)
    ...
    for (auto & i : point_cloud)
    {
        ...
        if (0 <= index && index < 30)
        {
            height_range[index]++;
            height_sum[index] += z;
            points_clusters[index].push_back(i);
        }
    }
    int max_num = 0;
    int max_index = -1;
    ...
    else
    {
        ...
        Vector3f tmp_p;
        tmp_p.setZero();
        for(const auto & inlier_point : inlier_points)
        {
            tmp_p += inlier_point;
        }
...
void DrawResult::drawGround(cv::Mat &result, vector<Vector3f> &point_cloud, Vector3f P_latest, Matrix3f R_latest)
{
    ...
    {
        Vector3f Pc;
        Pc = (R_latest * RIC).transpose()* (i - 1.0*P_latest  - R_latest * Vector3f(TIC_X,TIC_Y,TIC_Z));

        cv::Point2f pts;
        pts.x = FOCUS_LENGTH_X * Pc.x() / Pc.z()+ PX;
        pts.y = FOCUS_LENGTH_Y * Pc.y() / Pc.z()+ PY;

        points.emplace_back(pts.x, pts.y);
...
void DrawResult::drawBox(cv::Mat &result, const Vector3f& corner_0, const Vector3f& corner_x, const Vector3f& corner_y,
                         Vector3f corner_z, float size, Vector3f P_latest, Matrix3f R_latest, bool inAR) const
{
    ...
    boxConers.emplace_back(corner_0);
    boxConers.emplace_back(corner_x);
    boxConers.emplace_back(corner_y);
    boxConers.emplace_back(corner_x + corner_y - corner_0);

    boxConers.emplace_back(corner_z);
    boxConers.emplace_back(corner_x + corner_z - corner_0);
    boxConers.emplace_back(corner_y + corner_z - corner_0);
    boxConers.emplace_back(corner_x + corner_y + corner_z - 2.0*corner_0);

    vector<cv::Point2f> boxImage;
    Eigen::Vector3f Pc;
    vector<float> depth_of_coner;
    for(const auto& it : boxConers)
    {
        if (inAR)
...
void DrawResult::drawAR(cv::Mat &result, vector<Vector3f> &point_cloud, Vector3f P_latest, Matrix3f R_latest)
{
    ...
    {
        if ( Ground.boxflag)
            drawBox(result, Ground.ori, Ground.cox, Ground.coy, Ground.coz, Ground.size, P_latest, R_latest, true);
    }
    ...
    if ( locationX != locationX_p or locationY != locationY_p )
    {
        ...
        if ( dis_min < 120 and !Grounds.empty() and Grounds[dis_min_idx].moveflag and finger_state == 1)
        {
    ...
    if (longPressFlag)
    {
        ...
        if ( dis_min < 120 and !Grounds.empty() )
        {
    ...
    if (tapFlag and point_inlier.size()>28)
    {
        ...
        for (auto & Ground : Grounds)
        {
            Pc = (R_latest * RIC).transpose()* (Ground.center - 1.0*P_latest  - R_latest * Vector3f(TIC_X,TIC_Y,TIC_Z));
...
void DrawResult::drawBoxVirturCam(cv::Mat &result)
{
    ...
    for (auto & Ground : Grounds)
    {
        if ( Ground.boxflag)
        {
            drawBox(result, Ground.ori, Ground.cox, Ground.coy, Ground.coz, Ground.size, camInWorld_T, camInWorld_R, false);
        }
    }
}
...
/*
 ...
 Pv = RVC^t * Pc + TVC;
 */
cv::Point2f DrawResult::World2VirturCam(const Eigen::Vector3f& xyz, float &depth)
{
    Vector3f camInWorld_T;
    if(phy > 89)
        phy = 89;
...
void DrawResult::Reprojection(cv::Mat &result, vector<Vector3f> &point_cloud, const Matrix3f *R_window,const Vector3f *T_window, bool box_in_trajectorty)
{
    float depth_marker;
    cv::Mat aa;
    cv::resize(result, aa, Size(), 0.3, 0.3);//对原始图像进行缩放
    for (int i=0; i<result.rows; i++)
        for (int j=0; j<result.cols; j++)
        {
            result.at<cv::Vec3b>(i,j)[0] = 242;
            result.at<cv::Vec3b>(i,j)[1] = 242;
            result.at<cv::Vec3b>(i,j)[2] = 242;
        }

    Eigen::Matrix3f RIC;
    RIC(0,0)= 0;  RIC(0,1)= -1;  RIC(0,2)= 0;
    RIC(1,0)= -1; RIC(1,1)= 0;   RIC(1,2)= 0;
    RIC(2,0)= 0;  RIC(2,1)= 0;   RIC(2,3)= -1;
    ...
    //draw frame arrow
    {
        Vector3f p1, p2;
        cv::Point2f pt1, pt2;
        float length = 2.4f * 400 * radius / (Fx * 5.0) ;
        float scale_factor;
        p1 << -length, 0, 0;
        p2 << length, 0, 0;
        ...
        //draw grid
        {
            ...
            for(const auto& it : grid_space)
            {
                cv::Point2f pts1;
                pts1 = World2VirturCam(it.first, depth_marker);

                cv::Point2f pts2;
                pts2 = World2VirturCam(it.second, depth_marker);
                cv::line(result, pts1, pts2, cvScalar(180,180,180), 1, 8, 0);
            }
        }
    }

    //draw camera 滑动窗口内的相机位姿
    for(int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        camera_coner.reserve(5);
        for(const auto& it : camera_coner_w)
        {
    ...
    for (auto & i : point_cloud)
    {
        ...
        Pc = RIC.transpose()*i;
        Eigen::Vector3f Pv;
        pts = World2VirturCam(i, depth_marker);
        if(checkBorder(pts))
        {
    ...
    for (int i=0; i<aa.rows; i++)
        ...
        {
            result.at<cv::Vec3b>(result.rows - aa.rows + i,j)[0] = aa.at<cv::Vec3b>(i,j)[0];
            result.at<cv::Vec3b>(result.rows - aa.rows + i,j)[1] = aa.at<cv::Vec3b>(i,j)[1];
            result.at<cv::Vec3b>(result.rows - aa.rows + i,j)[2] = aa.at<cv::Vec3b>(i,j)[2];
        }
}
...
void DrawResult::drawPose(cv::Mat& result, const Matrix3f& r, const Vector3f& curTrans)
{
    float depth_marker;
    cv::Mat aa;
    cv::resize(result, aa, Size(), 0.3, 0.3);//对原始图像进行缩放
    ...
    for (int i=0; i<pose.size(); i++)
    {
        ...
        cv::line(result, pts_pre, pts, newColor(), 2, 8, 0);

        pts_pre = pts;
    }


    //draw frame arrow 画坐标系
    {
        Vector3f p1, p2;
        cv::Point2f pt1, pt2;
    ...
    //draw camera 相机位姿
    float length = 0.3;
    vector<Vector3f> camera_coner_w = calculate_camera_pose(curTrans, r, length);
    vector<cv::Point2f> camera_coner;
    camera_coner.reserve(5);
    for(const auto& it : camera_coner_w)
    {
